...
try {
    ...
    try {
        ...
        if ($null -eq $currentAccount) {
            throw "No account found where [$($correlationProperty)] = [$($correlationValue)]"           
        }
        else {
    ...
    foreach ($aRefProperty in $aRef.PSObject.Properties) {
        $exportData | Add-Member -MemberType NoteProperty -Name $aRefProperty.Name -Value $account.$($aRefProperty.Name) -Force
    }
